...
export class SAEO_Hardfile {
	constructor() {
		...
		/*---------------------------------*/
		function getchsgeometry(size, ptr, mode) {
			var spt, head, cyl;
		...
		function getchspgeometry(total, ptr, idegeometry) {
		...
		function hdf_read_target_2(hfd, buffer, buffero, offset, len) {
			if (offset == 0)
				hfd.cache_valid = 0;
		...
		/*---------------------------------*/
		function hdf_write_target_2(hfd, buffer, buffero, offset, len) {
			if (hfd.ci.readonly)
				return 0;
			//if (hfd.dangerous)
			//return 0;
