...
export class SAEO_CPU {
	constructor() {
		...
		function get32_dcache030(addr) {
			return read_dcache030(addr, 2);
		}
		function get16_dcache030(addr) {
			return read_dcache030(addr, 1);
		}
		function get8_dcache030(addr) {
		...
		function put32_dcache030(addr, v) {
			write_dcache030(addr, v, 2);
		}
		function put16_dcache030(addr, v) {
			write_dcache030(addr, v, 1);
		}
		function put8_dcache030(addr, v) {
			write_dcache030(addr, v, 0);
		}

		/*---------------------------------*/
		/* prefetch */
		function getInst16_icache020_prefetch(o) {
			var pc = getPC() + o;
			var v;
		...
		function getInst32_icache020_prefetch(o) {
			return ((getInst16_icache020_prefetch(o) << 16) | getInst16_icache020_prefetch(o + 2)) >>> 0;
		}

		function getInst16_icache030_prefetch(o) {
			var pc = getPC() + o;
			var v;
		...
		function getInst32_icache030_prefetch(o) {
		...
		function get8_prefetch(addr) {
			var v = SAER_Memory_get8(addr);
			regs.db = (v << 8) | v;
			return v;
		}
		function get16_prefetch(addr) {
			var v = SAER_Memory_get16(addr);
			regs.db = v;
			return v;
		}
		function get32_prefetch(addr) {
			return ((get16_prefetch(addr) << 16) | get16_prefetch(addr + 2)) >>> 0;
		}

		function put8_prefetch(addr, v) {
			regs.db = (v << 8) | v;
			SAER_Memory_put8(addr, v);
		}
		function put16_prefetch(addr, v) {
			regs.db = v;
			SAER_Memory_put16(addr, v);
		}
		function put32_prefetch(addr, v) {
		...
		function add_approximate_exception_cycles_020(nr) {
			var cycles;
		...
		/*-----------------------------------------------------------------------*/
		/* Condition codes */
		function flgAdd(S, D, R, m, isADDX) {
			var Sn = (S & m) != 0;
			var Dn = (D & m) != 0;
			var Rn = (R & m) != 0;
		...
		function flgSub(S, D, R, m, isSUBX) {
			var Sn = (S & m) != 0;
			var Dn = (D & m) != 0;
			var Rn = (R & m) != 0;
			regs.v = (!Sn && Dn && !Rn) || (Sn && !Dn && Rn);
