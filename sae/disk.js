...
export class SAEO_Disk {
	constructor() {
		...
		* when a real 3.5 HD disk is inserted. -Adil */
		...
		function update_drive_gui(num, force) {
			...
			if (!force && drv.state == gd.drive_motor[num]
				...
				&& gd.df[num] == SAEV_config.floppy.drive[num].file.name) return;
		...
		*/
		...
		this.insert = function (num, file) {
			//set_config_changed();
			//target_addtorecent(name, 0);
			disk_insert_2(num, file, false);
		};
		this.insert_force = function (num, file) {
		...
		this.update_adkcon = function (hpos, v) {
		...
		function examine_image(p, num, di) {
			var drv = floppy[num];
